...
class OrthogonalRegularizer(Regularizer):
  ...
  Arguments:
    factor: Float. The regularization factor. The regularization penalty will
      ...
      the rows (if `mode="rows"`, or columns if `mode="columns"`) of the inputs,
      excluding the product of each row/column with itself.
      Defaults to 0.01.
  ...
  def __call__(self, inputs):
    ...
    return self.factor * tf.reduce_sum(product_no_diagonal) / (2. * size)
