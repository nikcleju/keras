...
def layout_map_scope(layout_map):
  ...
  with layout_map_scope(layout_map):
    model = SubclassModel()
  # Trigger the build of weights within or outside of the scope works
  ...
  Yields:
    A context that all the tf.Variable created will be lazily init and
    finailized with a proper layout within the model.
  """
